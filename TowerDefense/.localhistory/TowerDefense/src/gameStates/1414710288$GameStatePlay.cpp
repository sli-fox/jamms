...
for(int i = 0; i < map.getMapWidth(); ++i) {
	for(int j = 0; j < map.getMapHeight(); ++j) {
		if(tower_manager.getTower(i,j) != NULL) {
			int distX = abs(blacky->getPosition().x-tower_manager.getTower(i,j)->getPosition().x);
			int distY = abs(blacky->getPosition().y-tower_manager.getTower(i,j)->getPosition().y);
