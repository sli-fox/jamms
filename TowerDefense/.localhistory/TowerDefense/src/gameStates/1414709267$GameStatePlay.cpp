...
void GameStatePlay::draw(const float delta_time) {
  ...
  if(!(range.getRadius() == 0))
	 this->game->game_window.draw(range);
...
for(int i = 0; i < map.getMapWidth(); ++i) {
	for(int j = 0; j < map.getMapHeight(); ++j) {
		if(tower_manager.getTower(i,j) != NULL) {
			int distX = abs(mew2->getPosition().x-tower_manager.getTower(i,j)->getPosition().x);
			int distY = abs(mew2->getPosition().y-tower_manager.getTower(i,j)->getPosition().y);
			int powdistX2distY2 = static_cast<int> (pow(static_cast<double> (distX), 2))
			+ static_cast<int> (pow(static_cast<double> (distY), 2));
			if(sqrt(powdistX2distY2)
			<= tower_manager.getTower(i,j)->getRangeRadius()) {
				std::cout << sqrt(powdistX2distY2) << "<=" << tower_manager.getTower(i,j)->getRangeRadius() << std::endl;
  ...
  while(this->game->game_window.pollEvent(event)) {
    ...
    switch(event.type) {
	  ...
	  case sf::Event::MouseMoved: {
		  // Display Tower's Range when hovered on
		  if(!tower_manager.outOfBound(tileX, tileY) && !tower_manager.isTileFree(tileX, tileY)) {
	      range = tower_manager.getTower(tileX, tileY)->displayRange(tileX*32, tileY*32);
		  }
		  else {
			  range = sf::CircleShape(0);
		  }
		  break;
		}
      default: break;
