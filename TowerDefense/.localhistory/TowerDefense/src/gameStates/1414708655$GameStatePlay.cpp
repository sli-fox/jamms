...
for(int i = 0; i < map.getMapWidth(); ++i) {
	for(int j = 0; j < map.getMapHeight(); ++j) {
		if(tower_manager.getTower(i,j) != NULL) {
			...
			if(sqrt( pow(distX, 2) + pow(distY, 2) ) <= tower_manager.getTower(i,j)->getRange()+1 * 32) {
				std::cout << sqrt( pow(distX, 2) + pow(distY, 2) ) << "<=" << tower_manager.getTower(i,j)->getRangeRadius() << std::endl;
