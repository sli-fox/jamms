...
GameStatePlay::GameStatePlay(Game* game) {
	GameStatePlay::tower_manager.setArraySize(map.getMapWidth(), map.getMapHeight());
  ...
  this->mew2 = new WhiteCat(getStartingWaypoint());
  this->game = game;
...
void GameStatePlay::draw(const float delta_time) {
  ...
  this->mew2->draw(this->game->game_window, delta_time);
  ...
  while(this->game->game_window.pollEvent(event)) {
    switch(event.type) {
	  ...
	  case sf::Event::KeyPressed: {
	   ...
	   mew2->controlCat(event.key.code);
...
void GameStatePlay::towerCommandLibrary(const int tileX, const int tileY, sf::Keyboard::Key thisKey){
	if(map.getTile(tileX, tileY)->getType() == Tile::TYPE::SCENERY) {
		if(thisKey == sf::Keyboard::Num1){
			tower_manager.buyTower(Tower::TowerType::WhiteBloodCellTower, tileX, tileY);	
		}
		if(thisKey == sf::Keyboard::Num2){
			tower_manager.buyTower(Tower::TowerType::AntibacterialTower, tileX, tileY);
		}
		if(thisKey == sf::Keyboard::Num3){
			tower_manager.buyTower(Tower::TowerType::ChemotherapyTower, tileX, tileY);
		}
		if(thisKey == sf::Keyboard::U){
			//tower_manager.tArray[tileX][tileY]->upgradeTower();
		}
		if(thisKey == sf::Keyboard::BackSpace){
			//tower_manager.tArray[tileX][tileY]->sellTower();
		}
...
TowerManager tower_manager = TowerManager::getInstance();
//TowerManager GameStatePlay::tower_manager(map.getMapWidth(),map.getMapHeight());