...
TowerManager::TowerManager(int mapWidth, int mapHeight) {
	const std::vector<std::vector<Tower*> > tArray(mapWidth, std::vector<Tower*>(mapHeight, NULL));
	std::cout << "Sizeee is: " << tArray.size() << "x" << tArray[0].size() << std::endl;;
...
Tower* TowerManager::getTower(int tileX, int tileY) {
	std::cout << "Size GSP: " << tArray.size() << "x" << std::endl;
