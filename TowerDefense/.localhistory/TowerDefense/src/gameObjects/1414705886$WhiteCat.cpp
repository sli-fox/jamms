...
void WhiteCat::controlCat(sf::Keyboard::Key arrowKey) {
	if(arrowKey == sf::Keyboard::Up) {
		this->updatePosition(this->sprite.getPosition().x,this->sprite.getPosition().x-3);
		std::cout << "Cat Up\n";
	}
	if(arrowKey == sf::Keyboard::Right) {
		this->updatePosition(this->sprite.getPosition().x+3,this->sprite.getPosition().y);
		std::cout << "Cat Right\n";
	}
	if(arrowKey == sf::Keyboard::Down) {
		this->updatePosition(this->sprite.getPosition().x,this->sprite.getPosition().y+3);
		std::cout << "Cat Down\n";
	}
