...
Tower::Tower() {
	...
	sf::CircleShape rangeShape((this->getRange()+1) * 32);
	rangeShape.setFillColor(sf::Color::Transparent);
	rangeShape.setOutlineThickness(2);
	rangeShape.setOutlineColor(sf::Color(100, 100, 100));
	rangeShape.setOrigin((this->getRange()*32), (this->getRange()*32));
	rangeShape.setPosition(this->getPosition().x-16, this->getPosition().y-16);
}
...
int Tower::getRangeRadius() {
	return (this->getRange()+1) * 32;
}
