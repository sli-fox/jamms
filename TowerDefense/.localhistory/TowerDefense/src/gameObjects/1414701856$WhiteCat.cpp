...
WhiteCat::WhiteCat() {
  // Set up animation
  Animation animation1(0, 2, 0.5f), animation2(0, 3, 0.5f), animation3(0, 3, 0.5f), animation4(0, 3, 0.5f);
  ...
  this->animation_handler.frame_size = sf::IntRect(0, 0, 32, 32);  //Add animations set in the constructor to handler
...
void WhiteCat::initializeCritter(const std::vector<Animation>& animations) {
  ...
  // Set WhiteCat texture
 // TextureManager& t_manager = TextureManager::getInstance();
  this->load("resources/images/WhiteCatSpriteSheet.png");
